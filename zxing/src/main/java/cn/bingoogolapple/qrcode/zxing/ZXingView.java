    ...
    protected ScanResult processData(byte[] data, int width, int height, boolean isRetry) {
        ...
        try {
            ...
            if (rawResult != null) {
                ...
                mBackBitmap = Bitmap.createBitmap(pixels, 0, width1, width1, height1, Bitmap.Config.ARGB_8888);
                ByteArrayOutputStream out = new ByteArrayOutputStream();
                mBackBitmap.compress(Bitmap.CompressFormat.JPEG, 100, out);
                mBackBitmap = BitmapFactory.decodeByteArray(out.toByteArray(), 0, out.toByteArray().length, null);
                mBackBitmap = mBackBitmap.copy(Bitmap.Config.ARGB_8888, true);
            }
        ...
        if (isShowLocationPoint() || isNeedAutoZoom) {
            ...
            if (transformToViewCoordinates(pointArr, scanBoxAreaRect, isNeedAutoZoom, result,mBackBitmap)) {
                return null;
            }
        }
        return new ScanResult(result, mBackBitmap);
    }
