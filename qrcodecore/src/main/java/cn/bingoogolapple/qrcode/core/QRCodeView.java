...
public abstract class QRCodeView extends RelativeLayout implements Camera.PreviewCallback {
    ...
    void onPostParseData(ScanResult scanResult) {
        ...
        if (TextUtils.isEmpty(result)) {
            ...
            try {
                if (mDelegate != null) {
                    mDelegate.onScanQRCodeSuccess(result,scanResult);
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    protected boolean transformToViewCoordinates(final PointF[] pointArr, final Rect scanBoxAreaRect, final boolean isNeedAutoZoom, final String result, Bitmap mBackBitmap) {
        ...
        try {
            ...
            if (isNeedAutoZoom) {
                return handleAutoZoom(transformedPoints, result,mBackBitmap);
            }
            return false;
        } catch (Exception e) {
            mLocationPoints = null;
    ...
    private boolean handleAutoZoom(PointF[] locationPoints, final String result, final Bitmap mBackBitmap) {
        ...
        post(new Runnable() {
            @Override
            public void run() {
                startAutoZoom(zoom, Math.min(zoom + zoomStep, maxZoom), result,mBackBitmap);
            }
        });
        return true;
    }

    private void startAutoZoom(int oldZoom, int newZoom, final String result, final Bitmap mBackBitmap) {
        ...
        mAutoZoomAnimator.addListener(new AnimatorListenerAdapter() {
            ...
            public void onAnimationEnd(Animator animation) {
                onPostParseData(new ScanResult(result,mBackBitmap));
            }
        });
    ...
    public interface Delegate {
         ...
         * @param scanResult
         */
        void onScanQRCodeSuccess(String result, ScanResult scanResult);
